...
OLLAMA_MODEL = "qwen2.5-coder:7b"  # 修改此处切换模型
...
7. 最终分析结果必须保存到变量 result。
8. 如果需要绘图，请使用 matplotlib。
9. 不要调用 plt.show()。
10. 图表标题、X轴、Y轴、图例等所有中文文字必须使用：
    fontproperties=CN_FONT
11. 不要修改原始 DataFrame，如需处理，请使用新的变量。
12. 如果结果是 DataFrame、Series、数字或字符串，都放入变量 result。
13. 如果需要排序，请明确指定 ascending=True 或 False。
14. 不要使用不存在的字段，如果字段不存在，请根据已有字段进行分析。
15. 尽量编写简洁、可读、符合 pandas 最佳实践的代码。
...
"""
        ...
        with st.spinner("Ollama分析中..."):
            ...
            code = api_response.json()["response"]
