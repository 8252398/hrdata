...
with st.sidebar:
    ...
    if submitted:
        if ds_token_input:
            st.session_state.ds_token = ds_token_input
            st.success("Token 已保存")
        else:
            st.warning("请输入 Token")
if not st.session_state.ds_token:
    st.info("👈 请在左侧输入 DeepSeek API Key 后开始使用")
    st.stop()
...
if uploaded_files:
    ...
    for f in uploaded_files:
        ...
        if f.name.endswith(".csv"):
            ...
            dataframes[name] = pd.read_excel(f)

    st.subheader(f"已加载 {len(dataframes)} 个文件")
    ...
    if st.button("开始分析"):
        ...
        df_descriptions = []
        for name, df in dataframes.items():
            df_descriptions.append(f"""变量名: {name}
字段: {list(df.columns)}
前5行:
{df.head().to_markdown()}
""")
        all_df_info = "\n---\n".join(df_descriptions)
        df_name = list(dataframes.keys())[0] if dataframes else ""
...
avg_score = {df_name}.groupby("部门")["成绩"].mean()
